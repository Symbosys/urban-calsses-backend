    ...
    const course = await prisma.course.create({
        data: {
            ...
            instructors: instructorIds?.length
                ? {
                    create: instructorIds.map((instructorId) => ({
                        instructorId,
                    })),
                }
                : undefined,
...
export const updateCourse = asyncHandler(async (req, res, next) => {
    ...
    if (req.file) {
        ...
        if (existingCourse.thumbnail && existingCourse.thumbnail.public_id) {
            await deleteFromCloudinary(existingCourse.thumbnail.public_id);
        }
        const { public_id, secure_url } = await uploadToCloudinary(req.file.buffer, "courses");
        thumbnailData = { public_id, secure_url };
    }
    ...
    const updatedCourse = await prisma.course.update({
        ...
        data: {
            ...
            slug,
            thumbnail: thumbnailData,
            instructors: instructorIds !== undefined
                ? {
                    deleteMany: {},
                    create: instructorIds.map((instructorId) => ({
                        instructorId,
