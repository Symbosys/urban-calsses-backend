...
export const getMyReview = asyncHandler(async (req, res, next) => {
  const courseId = req.params.courseId as string;
  const userId = req.query.userId as string; // Taking from query for now
...
export const updateReview = asyncHandler(async (req, res, next) => {
  ...
  const updateData: any = {};
  if (validation.rating !== undefined) updateData.rating = validation.rating;
  if (validation.comment !== undefined) updateData.comment = validation.comment;
