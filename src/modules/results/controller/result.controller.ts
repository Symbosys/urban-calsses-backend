...
export const updateResult = asyncHandler(async (req, res, next) => {
  ...
  const existing = await (prisma as any).result.findUnique({ where: { id } });
  if (!existing) return next(new ErrorResponse("Result not found", 404));
  ...
  const result = await (prisma as any).result.update({
    where: { id },
    data: {
      ...updateData,
      image: imageData,
...
export const deleteResult = asyncHandler(async (req, res, next) => {
  ...
  await (prisma as any).result.delete({ where: { id } });
